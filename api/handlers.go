...
func CreateUser(w http.ResponseWriter, r *http.Request, p httprouter.Params) {
	res, _ := ioutil.ReadAll(r.Body) //从request的body里读出信息res
	...
	su := &defs.SignedUp{ //为序列化响应体做准备
		Success:   true,
		SessionId: id,
	}
	//序列化响应体
	if resp, err := json.Marshal(su); err != nil {
		sendErrorResponse(w, defs.ErrorInternalFaults) //错误响应
		return
	} else {
		sendNormalResponse(w, string(resp), 201) //正常响应
	}
}
...
func Login(w http.ResponseWriter, r *http.Request, p httprouter.Params) {
	...
	io.WriteString(w, "signed in") //上线后省略
}
